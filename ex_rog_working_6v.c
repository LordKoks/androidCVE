...
shell:
    ...
    // CRITICAL: Bionic (Android) linker doesn't understand libtermux-exec-ld-preload.so
    // If LD_PRELOAD is set to that, /system/bin/sh fails with "library not found".
    // We must UNSET it before exec'ing any Bionic binary.
    unsetenv("LD_PRELOAD");
    ...
    for (int i = 0; sh_paths[i]; i++) {
        if (access(sh_paths[i], X_OK) == 0) {
            // For Termux shell, re-set LD_PRELOAD; for system shells, ensure it's unset
            if (strstr(sh_paths[i], "com.termux") != NULL) {
            ...
            fprintf(stderr, "[CHILD %d] Trying shell: %s (LD_PRELOAD=%s)\n", getpid(), sh_paths[i], getenv("LD_PRELOAD") ? "set" : "unset");
            fflush(stderr);
    ...
    // Last resort
    fprintf(stderr, "[CHILD %d] All shells failed, exit\n", getpid());
    fflush(stderr);
    exit(0);
}
